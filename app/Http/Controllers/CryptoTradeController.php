...
class CryptoTradeController extends Controller
{
    ...
    public function index(Request $request, string $crypto)
    {
        return Inertia::render('CryptoTrade', [
            ...
            'displayClock' => (bool)$request->input('displayClock') ?? true,
            'displayMoon' => (bool)$request->input('displayMoon') ?? true,
            'displaySpaceship' => (bool)$request->input('displaySpaceship') ?? true,
            'displayXTicks' => (bool)$request->input('displayXTicks') ?? false,
            'tooltip' => (bool)$request->input('tooltip') ?? false,
